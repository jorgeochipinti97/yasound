...
export const Navbar = () => {
  ...
  const tabsComponents = pillTabs.map((text, i) => {
    return (
      <button
        className="p-2  border-2 text-black  text-xs font-extralight  md:text-xl font-sans rounded-full bg-white"
        onClick={() =>
          (text == "Home" && push("/")) ||
          (text == "Beats" && push("#")) ||
  ...
  return (
    <div>
        ...
        {user && (
          <DropdownMenu className='indexz_'>
            ...
            <DropdownMenuContent>
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem
                onClick={() => push(`/create/${usuario.username}`)}
              >
                Editar Perfil
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => push(`/perfil/${usuario.username}`)}
              >
                Ver perfil
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => push(`/upload`)}>
                Tu musica
              </DropdownMenuItem>
      ...
      <div className="fixed right-1 bottom-1 md:hidden p-3 indexz_ bg-white rounded-full flex justify-center">
          {user ? (
            <div className="flex  " onClick={() => logout()}>
              <div className=" transition-all hover:scale-150 cursor-pointer">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
      ...
      >
        ...
        <div className="md:flex  z-50 hidden items-center mx-5 ">
          {user && (
            <DropdownMenu>
              <DropdownMenuTrigger>
                <div className="flex justify-center items-center flex-col">
